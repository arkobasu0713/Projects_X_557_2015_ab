...
vec4 useLight(Light light, vec4 surfacePostion, vec4 normal_transformed, vec3 normal, Material material )
{
    ...
    if(light.light_position.w == 1.0)
    {
        ...
        light.spotEffect = 1 - smoothstep(light.cone_angle - 4.3, light.cone_angle, light_to_surface_angle);
		}
    }
    ...
    vec3 linearColor = out_ambient_color  + light.spotEffect * attenuation * ( out_diffuse_color + out_specular_color);
